...
%
\begin{proof}
For all $x \in \mathcal{X}$, let $K_x : \mathcal{X} \to \mathbb{R}$ be the function such that $K_x(y) = K(x, y)$. The set of all linear combinations of these functions forms a vector space $\mathcal{H}$.

To turn $\mathcal{H}$ into a Hilbert space, we first define 
\end{proof}
