...
We now capture this intuition formally using Cauchy sequences. Given a vector space $\mathcal{V}$ over $\mathcal{R}$, let $d : \mathcal{V} \times \mathcal{V} \to \mathbb{R}$ be a function that takes two vectors and computes their ``distance''. This function must satisfy the following axioms for all $\mathbf{u}, \mathbf{v}, \mathbf{w} \in \mathcal{V}$:
...
Equipped with this distance function, we now can define a Cauchy sequence. A sequence $\langle x_i \rangle_{i=0}^\infty$ is \defn{Cauchy} if for every real number $\epsilon > 0$ there is a positive integer $N$ such that for all positive integers $m, n \geq N$, $d(x_m, x_n) < \epsilon$. In other words, the members of the sequence get closer and closer to each other in terms of the distance function until they converge to a limit. A space is \defn{complete} if every Cauchy sequence converges to an element in the space. Continuing our example in $(0, 1]$, the sequence $x_i = \frac{1}{n}$ is Cauchy and converges to $0$. However, $0$ is not in that set, so $(0, 1]$ is not complete.
...
We can define the distance function as $d(\mathbf{u}, \mathbf{v}) = \normLL{\mathbf{u} - \mathbf{v}}$.

\begin{definition}[Banach space]
A Banach space is a normed vector space that is complete with respect to the distance function induced by the norm.
\end{definition}
