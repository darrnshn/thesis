...
Our \todo{approach?} is \todo{inspired?} by this technique. Instead of working with probability distributions directly, we embed them in a reproducing kernel Hilbert space (RKHS). Mapping to a Hilbert space has several benefits:
%
\begin{itemize}
...
A \defn{linear functional} is a linear function from a vector space to its field.
...
Let $K_y$ be any function in $\mathcal{H}$. By the reproducing property, $K_y(x) = \innerProd{K_y, K_x}_\mathcal{H}$ for every $x \in \mathcal{H}$. We can then define the \defn{reproducing kernel} of $\mathcal{H}$ as $K(x, y) \defeq K_x(y) = \innerProd{K_x, K_y}_\mathcal{H}$.
...
Let $\phi(\mathbf{x}) \defeq K(\mathbf{x}, \cdot)$
\begin{align}
...
The function $\phi$ can be thought of as an implicit \defn{feature map} from the input space $\mathcal{X}$ to an inner product space $\mathcal{V}$ called the \defn{feature space}. So evaluating a kernel can be thought as way of computing the inner product in a Hilbert space without explicitly mapping between input space and feature space. This technique is known as the \defn{kernel trick}, which is commonly used in non-linear support vector machines.
