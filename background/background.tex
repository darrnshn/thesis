...
Suppose we are working with some abstract set $\mathcal{S}$ (for instance, a set of probability distributions). We have two choices: we can either

\begin{enumerate}
  \item Develop the necessary theory to work directly with $S$.
  \item Map elements of $S$ to another set $S^\prime$ and work within the new set $S^\prime$ instead.
\end{enumerate}

The latter choice is advantageous if the new set $S^\prime$ is more convenient to work with than $S$. If we work in $S^\prime$, any properties, theorems, computational savings, and intuition we have for $S^\prime$ will be available to us. All we need is a ``embedding'' or ``representation theorem'' to transform elements of $S$ to $S^\prime$.

This approach underpins the theory in this thesis. Instead of working with probability distributions directly, we embed them into a reproducing kernel Hilbert space (RKHS) which makes them easier and more intuitive to manipulate.
...
A \defn{vector space} over a field $\mathcal{F}$ consists of a set $\mathcal{V}$, a \defn{vector addition} operation $\oplus$, and a \defn{scalar multiplication} operation $\otimes$. It must obey the following axioms for all $\mathbf{u}, \mathbf{v}, \mathbf{w} \in \mathcal{V}$ and $a, b \in \mathcal{F}$:
\begin{description}
  ...
  \item[Existence of Identity] There exists elements $\mathbf{0} \in \mathcal{V}$ and $1 \in \mathcal{F}$ such that $\mathbf{0} \oplus \mathbf{u} = \mathbf{u}$ and $1 \otimes \mathbf{u} = \mathbf{u}$.
...
A inner product space is a vector space with an extra operation called the \defn{inner product}: $\innerProd{\cdot, \cdot} : \mathcal{V} \times \mathcal{V} \to \mathcal{F}$ that satisfies the following axioms for all $\mathbf{x}, \mathbf{y}, \mathbf{z} \in \mathcal{V}$ and $a \in \mathcal{F}$:
\begin{description}
  ...
  \item[Positive-definiteness] $\innerProd{\mathbf{v}, \mathbf{v}} \geq 0$, equality holds when $\mathbf{v} = \mathbf{0}$.
